...
export default function TalepForm({ onSubmit }: TalepFormProps) {
  ...
  const validateForm = () => {
    ...
    if (!formData.talepSahibi) newErrors.push("Talep Sahibi seçilmelidir")
    if (!formData.talepSahibiAciklamasi) newErrors.push("Talep Sahibi Açıklaması seçilmelidir")
    if (
      formData.talepSahibi === "Dış Paydaş" &&
      formData.talepSahibiAciklamasi === "Diğer" &&
      !formData.talepSahibiDigerAciklama
    ) {
      newErrors.push("Diğer Açıklama alanı doldurulmalıdır")
    }
    if (!formData.talepIlcesi) newErrors.push("Talep İlçesi seçilmelidir")
    if (!formData.bolge) newErrors.push("Bölge seçilmelidir")
    if (!formData.hatNo.trim()) newErrors.push("Hat No girilmelidir")
    if (!formData.isletici) newErrors.push("İşletici seçilmelidir")
    if (!formData.talepOzeti.trim()) newErrors.push("Talep Özeti girilmelidir")
    if (!formData.talepIletimSekli) newErrors.push("Talep İletim Şekli seçilmelidir")
    if (!formData.yapılanIs.trim()) newErrors.push("Talebe İlişkin Yapılan İş girilmelidir")
    if (!formData.talepDurumu) newErrors.push("Talep Durumu seçilmelidir")

    // EBYS seçilmişse evrak bilgileri zorunlu
    if (formData.talepIletimSekli === "Elektronik Belge Yönetim Sistemi (EBYS)") {
      if (!formData.evrakTarihi) newErrors.push("EBYS seçildiğinde Evrak Tarihi girilmelidir")
      if (!formData.evrakSayisi.trim()) newErrors.push("EBYS seçildiğinde Evrak Sayısı girilmelidir")
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validationErrors.length > 0) {
      // Sayfayı en üste kaydır ki kullanıcı hataları görebilsin
  ...
  const handleInputChange = (field: string, value: string) => {
    ...
    // Hata listesinden bu alanla ilgili hataları temizle
    if (errors.length > 0) {
      setErrors([])
    }
  }
    ...
    <div className="space-y-6">
      ...
      {errors.length > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <div className="font-semibold mb-2">Lütfen aşağıdaki alanları kontrol edin:</div>
